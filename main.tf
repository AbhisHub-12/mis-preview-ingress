locals {
  ...
  # FINAL FIX: Use MD5 hash to ensure unique service names and prevent any collisions
  external_services = {
    for k, v in local.ingressObjectsFiltered :
    k => {
      namespace = lookup(v, "namespace", var.environment.namespace)
      # FINAL ENHANCEMENT: Use MD5 hash of the unique combination to prevent ANY collisions
      service_name  = substr("${var.instance_name}-ext-${substr(md5("${v.service_name}-${lookup(v, "namespace", var.environment.namespace)}-${k}"), 0, 8)}", 0, 63)
  ...
  # FINAL FIX: Enhanced default backend handling with MD5 hash for uniqueness
  # Create a list of all default backends with their details including rule keys for uniqueness
  default_backend_list = [
    for key, value in local.header_based_routing_with_default_backend : {
      key = key
      # FINAL ENHANCEMENT: Use MD5 hash to ensure absolute uniqueness
      backend_key  = "${var.instance_name}-${substr(md5("${lookup(lookup(value, "header_based_routing", {}), "default_backend", "")}-${lookup(lookup(value, "header_based_routing", {}), "default_backend_namespace", var.environment.namespace)}-${key}"), 0, 8)}"
  ...
  # Group by backend_key to identify unique backends (now with enhanced uniqueness)
  grouped_backends = {
    for backend in local.default_backend_list : backend.backend_key => backend...
  }

  # Create a map of unique default backend services with enhanced naming
  unique_default_backends = {
    for backend_key, backends in local.grouped_backends : backend_key => {
      namespace = backends[0].namespace
      # FINAL ENHANCEMENT: Use MD5 hash for service names to prevent collisions, truncated to 63 chars
      service_name  = substr("${var.instance_name}-ext-${substr(md5("${backends[0].service_name}-${backends[0].namespace}-${backends[0].key}"), 0, 8)}", 0, 63)
...
resource "kubernetes_secret" "ingress-auth" {
  ...
  lifecycle {
    ignore_changes        = [data] # FIXED: Removed quotes
...
# FIXED: Conditional AWS Route53 records - only create when AWS provider is available
resource "aws_route53_record" "cluster-base-domain" {
  count = can(var.cc_metadata.tenant_base_domain_id) && local.tenant_provider == "aws" && !lookup(var.instance.spec, "disable_base_domain", false) ? 1 : 0
  depends_on = [
    helm_release.nginx_ingress_ctlr
  ]
    ...
    for key, value in local.header_based_routing_with_default_backend : "${key}-default" => {
      ...
      spec = {
        ...
        tls = [{
          ...
          secretName = lookup(lookup(value, "custom_tls", {}), "enabled", false) ? lower("${value.domain_key}-custom-tls") : local.disable_endpoint_validation ? (trimspace(lookup(value, "certificate_reference", "")) == "" ? null : lookup(value, "certificate_reference", "")) : lower(join("-", compact([var.instance_name, value.domain_key, replace(lookup(value, "domain_prefix", ""), "*", "")])))
        }]
      }
    }
  }
  ...
  ingress_resources = {
    for key, value in local.ingressObjectsFiltered : key => {
      ...
      spec = {
        ...
        rules = [{
          ...
          http = {
            paths = [{
              ...
              backend = {
                service = {
                  # FIXED: Use the enhanced service name mapping for external services
                  name = contains(keys(local.external_services), key) ? local.external_services[key].service_name : value.service_name
                  port = {
                    name = lookup(value, "port_name", null)
                    number = lookup(value, "port_name", null) != null ? null : (
                      lookup(value, "port", null) != null ? tonumber(lookup(value, "port", null)) : null
        ...
        tls = [{
          hosts      = local.disable_endpoint_validation ? tolist([lookup(value, "domain", null), "*.${lookup(value, "domain", null)}"]) : tolist([value.host])
          secretName = lookup(lookup(value, "custom_tls", {}), "enabled", false) ? "${value.domain_key}-custom-tls" : local.disable_endpoint_validation ? lookup(value, "certificate_reference", "") == "" ? null : lookup(value, "certificate_reference", "") : trimspace(replace(lookup(value, "domain_prefix", ""), "*", "")) == "" ? lower("${var.instance_name}-${value.domain_key}") : lower("${var.instance_name}-${value.domain_key}-${replace(lookup(value, "domain_prefix", ""), "*", "")}")
        }]
      }
    }
  }
...
module "ingress_resources" {
  ...
  name            = length("${lower(var.instance_name)}-${each.key}") > 53 ? "${substr("${lower(var.instance_name)}-${each.key}", 0, 45)}-${substr(md5(each.key), 0, 7)}" : "${lower(var.instance_name)}-${each.key}"
  namespace       = var.environment.namespace
  advanced_config = {}
  data            = each.value
}
...
# FINAL FIX: Enhanced external service creation with MD5 hash collision prevention
...
# FINAL FIX: Enhanced default backend external service creation with MD5 hash collision prevention  
